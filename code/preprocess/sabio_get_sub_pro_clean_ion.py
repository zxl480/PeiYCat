...
def update_substrate_product(json_data):
    ...
    for entry_id, entry in json_data.items():
        ...
        for sub in substrate.split(";"):
            match = re.search(rf'(?:^|\s)(\d*)\s*{re.escape(sub)}(?:\s|$)', reaction)
            if match:
                count = int(match.group(1)) if match.group(1) else 1
                if count > 1:
                    coefficient_updates += 1
                updated_substrate.extend([sub] * count)
            else:
        ...
        for prod in product.split(";"):
            match = re.search(rf'(?:^|\s)(\d*)\s*{re.escape(prod)}(?:\s|$)', reaction)
            if match:
                count = int(match.group(1)) if match.group(1) else 1
                if count > 1:
                    coefficient_updates += 1
                updated_product.extend([prod] * count)
            else:
...
with open('../../data/preprocess/sabio_data_only_one_uniprotId.json', 'r', encoding='utf-8') as f:
...
with open("../../data/preprocess/sabio_data_get_sub_pro_clean_ion.json", 'w') as f:
